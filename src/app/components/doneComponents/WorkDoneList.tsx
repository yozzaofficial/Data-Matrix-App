...
export default function WorkDoneList({
  setOpenWorkDetail,
  item,
}: WorkDoneListProps) {
  // prima era toDoValue
  ...
  async function openWorkDetail(id: string) {
    if (filterTime !== null) router.replace(`done?time=${filterTime}&id=${id}`);
    else if (filterTech !== null)
      router.replace(`done?name=${filterTech}&id=${id}`);
    else router.replace(`done?id=${id}`);
  ...
  function getDataToRender() {
    if (filterTime !== null) return filteredDataTime;
  ...
  const liElements = dataToRender.map((d) => {
    return (
      <li key={d.id} className="workDone" onClick={() => openWorkDetail(d.id)}>
        {d.nameitem}
  ...
  return (
    <ul className="workDoneList">
      {liElements.length > 0 ? (
        ...
        <p className="noWorkDone">No work done yet</p>
      )}
    </ul>
  );
}
