...
export default function WorkList({
  ...
  React.useEffect(() => {
    //set filter emergency default
    if (!filter) {
      if (filterId !== "")
        router.replace(`todo?filter=emergency&id=${filterId}`);
      else router.replace(`todo?filter=emergency`);
    }
  }, [filter, router]);
  ...
  const liElements = filteredData.map((e) => (
    <li key={e.id} className="work" onClick={() => openWorkDetail(e.id)}>
      {e.name}
  ...
  return <ul className="workList">{liElements}</ul>;
}
